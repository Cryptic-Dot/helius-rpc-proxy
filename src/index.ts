...
export default {
	async fetch(request: Request, env: Env) {
		...
		if (upgradeHeader || upgradeHeader === 'websocket') {
			return await fetch(`https://intensive-delicate-shape.solana-mainnet.quiknode.pro/${env.QUICKNODE_API_KEY}`, request)
		}


		const { pathname, search } = new URL(request.url)
		const payload = await request.text();
		const proxyRequest = new Request(`https://intensive-delicate-shape.solana-mainnet.quiknode.pro/${env.QUICKNODE_API_KEY}${pathname}${search ? `&${search.slice(1)}` : ''}`, {
